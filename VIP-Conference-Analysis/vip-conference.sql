...
-- Query 1- Which VIPs are presenting at events and what is the event? 
-- List all VIPs and the events they are presenting
SELECT 
    vips.first_name,
    vips.last_name,
    events.event_name,
...
-- Query 2- Which VIPs have RSVP’d for welcome or dinner at a hotel?
SELECT 
	vips.first_name, 
    vips.last_name,
    hotels.hotel_name,
...
-- Query 3 - Who is giving feedback to who and which events are involved 
SELECT 
    v1.first_name AS reviewer_first_name,
    v1.last_name AS reviewer_last_name,
    v2.first_name AS recipient_first_name,
    v2.last_name AS recipient_last_name,
    e.event_name AS event_name
FROM vips v1
INNER JOIN vips v2 ON v1.provides_feedback_to = v2.member_id
LEFT JOIN events e ON v1.event = e.event_id
ORDER BY v1.last_name;  

SELECT 
    v1.first_name AS vip_name,
    v1.last_name AS vip_last,
    v2.first_name AS feedback_to_name,
    v2.last_name AS feedback_to_last,
    e.event_name
FROM vips v1
INNER JOIN vips v2
...
-- Query 4 - Which hotel has the most VIP reservations?
SELECT 
	hotel, 
...
-- Query 5 - Which VIPs belong to the same organizations?
SELECT 
    v1.first_name AS vip1_name,
    v1.last_name AS vip1_last,
    v2.first_name AS vip2_name,
    v2.last_name AS vip2_last,
    v1.association
FROM vips v1
INNER JOIN vips v2
	ON v1.association = v2.association
	AND v1.member_id < v2.member_id
ORDER BY v1.association;

-- Query 6 - Which events have VIPs assigned but no feedback provided yet?
SELECT 
    e.event_name,
    e.event_type,
    v.first_name,
...
WHERE v.provides_feedback_to IS NULL;
